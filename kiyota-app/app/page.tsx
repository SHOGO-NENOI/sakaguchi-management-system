...
const SPREADSHEET_NAME = "清田さん 勤怠記録データ";
...
export default function Home() {
  ...
  const summary = useMemo(
    () =>
      summaryEntries.reduce(
        ...
        { days: 0, work: 0, early: 0, overtime: 0, trips: 0 },
      ),
    [summaryEntries],
  );
  ...
  async function removeMany(ids: string[]) {
    ...
    if (!response.ok) setError("一部を削除できませんでした");
  }
  ...
  async function toggleMasterArchive(
    ...
    const data = await response.json();
    if (!response.ok) {
      setMasterMessage(data.error || "更新できませんでした");
      return;
    }
    setMasterOptions((current) => ({
      ...current,
  ...
  async function editTool(
    ...
    const data = await response.json();
    if (!response.ok) {
      setToolMessage(data.error || "編集できませんでした");
      return;
    }
    setToolSets((current) =>
      current.map((set) =>
  ...
  async function archiveSite(card: SiteCardData) {
    ...
    const data = await response.json();
    if (!response.ok) {
      setSiteNameMessage(data.error || "非表示にできませんでした");
      return;
    }
    setSiteMasters((current) =>
      current.map((site) => (site.id === card.masterId ? data.site : site)),
      ...
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            ...
            <small>清田さん専用｜勤怠・現場・道具を一元管理</small>
          </div>
        </div>
          ...
          <section className="sync-manager-card">
            <div className="section-heading">
              <div>
                ...
                <h2>Google同期管理</h2>
                <p>
                  アプリで削除するとGoogle側も削除します。Google側だけで削除してもアプリの記録は残ります。
                </p>
              </div>
              <span className="sync-live">● 同期オン</span>
            </div>
            {!syncDashboard ? (
              ...
              <>
                ...
                {syncDashboard.duplicates.length > 0 && (
                  <details className="sync-detail" open>
                    <summary>重複予定の整理</summary>
                    <div>
                      {syncDashboard.duplicates.flatMap((group) =>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="record-extra">
                        ...
                        {entry.businessTrip && (
                          <span className="trip-badge">出張</span>
                        )}
