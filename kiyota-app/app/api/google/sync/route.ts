    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (linked.some((event) => event.status === "cancelled") || linked.length < ids.length) {
          await db.update(attendanceEntries).set({ syncStatus: "error", syncError: "Googleカレンダー側の予定が削除されています。再実行すると予定を作り直します", lastSyncedAt: new Date().toISOString(), lastModifiedSource: "google" }).where(eq(attendanceEntries.id, row.id));
