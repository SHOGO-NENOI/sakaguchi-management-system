...
import { createAuthorizedSession, KIYOTA_EMAIL, KIYOTA_SESSION_COOKIE } from "../../../authorized-user";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (profile.email.trim().toLowerCase() !== KIYOTA_EMAIL) {
      await fetch(`https://oauth2.googleapis.com/revoke?token=${encodeURIComponent(token.access_token)}`, { method: "POST" }).catch(() => undefined);
      throw new Error(`清田さんのGoogleアカウント（${KIYOTA_EMAIL}）でログインしてください`);
    }
    ...
    session = await createAuthorizedSession(profile.email);
  } catch (error) {
  ...
  headers.append("Set-Cookie", oauthCookie(KIYOTA_SESSION_COOKIE, session, session ? 30 * 24 * 60 * 60 : 0));
