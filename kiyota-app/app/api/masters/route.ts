...
type Payload = { action?: "add" | "edit" | "archive" | "restore"; type?: MasterType; id?: number; name?: string };
...
const people = (value: string) => parts(value).flatMap((part) => part.split(/[、,，\n]/).map((name) => name.trim()).filter(Boolean));
...
async function seedHistory(type: MasterType) {
  ...
  const names = [...new Set(rows.flatMap((row) => type === "work" ? parts(row.work) : people(row.personnelNames)))];
  const now = new Date().toISOString();
  for (let index = 0; index < names.length; index += 1) await db.insert(masterOptions).values({ type, name: names[index], sortOrder: index, createdAt: now, updatedAt: now }).onConflictDoNothing();
}

export async function GET(request: Request) {
  const authError = await requireAuthorizedApiUser(); if (authError) return authError;
  try {
    const type = new URL(request.url).searchParams.get("type"); if (!validType(type)) throw new Error("マスターの種類が正しくありません");
    await seedHistory(type); const db = await getDb();
...
export async function POST(request: Request) {
  const authError = await requireAuthorizedApiUser(); if (authError) return authError;
  try {
    const body = await request.json() as Payload; if (!validType(body.type)) throw new Error("マスターの種類が正しくありません");
    ...
    if (body.action === "add") {
      ...
      const [option] = await db.insert(masterOptions).values({ type: body.type, name, sortOrder: (last?.sortOrder ?? -1) + 1, createdAt: now, updatedAt: now }).returning(); return Response.json({ option }, { status: 201 });
    }
    ...
    if (body.action === "edit") {
      ...
      const [option] = await db.update(masterOptions).set({ name, updatedAt: now }).where(and(eq(masterOptions.id, body.id), eq(masterOptions.type, body.type))).returning(); return Response.json({ option });
    }
    if (body.action === "archive" || body.action === "restore") {
      const [option] = await db.update(masterOptions).set({ archivedAt: body.action === "archive" ? now : "", updatedAt: now }).where(and(eq(masterOptions.id, body.id), eq(masterOptions.type, body.type))).returning(); return Response.json({ option });
    }
    throw new Error("操作内容が正しくありません");
  } catch (error) { const message = error instanceof Error ? error.message : "保存できませんでした"; return Response.json({ error: message.includes("UNIQUE") ? "同じ名前がすでにあります" : message }, { status: 400 }); }
}
