...
export async function syncSheet(row: EntryRow, action: "upsert" | "delete") {
  ...
  const values = [[String(row.id), row.workDate, status, normalizedWorkType(row.workType), row.startTime, row.endTime, row.location, row.site, row.address, row.coordinates, row.personnelNames, row.work, row.note, row.businessTrip ? "出張" : "", row.hotelName, new Date().toISOString()]];
  if (rowNumber > 0) {
    await googleFetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheet.id}/values/${encodeURIComponent(`勤務記録!A${rowNumber}:P${rowNumber}`)}?valueInputOption=USER_ENTERED`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ values }) });
  } else {
    await googleFetch(`https://sheets.googleapis.com/v4/spreadsheets/${spreadsheet.id}/values/${encodeURIComponent("勤務記録!A:P")}:append?valueInputOption=USER_ENTERED`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ values }) });
  }
  return spreadsheet.url;
}
...
export async function syncGoogleCalendar(row: EntryRow, action: "upsert" | "delete", options: { calendarEndDate?: string; suppressCalendar?: boolean } = {}) {
  ...
  for (const [targetIndex, target] of targets.entries()) {
    ...
    const event = isOff ? { summary: row.note.trim() || "休み", colorId: "2", start: { date: row.workDate }, end: { date: addDay(options.calendarEndDate || row.workDate) }, extendedProperties: identity } : {
      ...
      description: [`記録状態：${status}`, `勤務区分：${workType}`, `現場：${place}`, target.site.work && `作業内容：${target.site.work}`, target.site.address && `住所：${target.site.address}`, target.site.coordinates && `緯度・経度：${target.site.coordinates}`, target.site.personnelNames && `作業者：${target.site.personnelNames}`, row.businessTrip && row.hotelName && `宿泊ホテル：${row.hotelName}`, target.site.note && `メモ：${target.site.note}`].filter(Boolean).join("\n"), colorId: row.businessTrip ? "5" : "7",
