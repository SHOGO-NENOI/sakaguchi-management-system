...
export async function ensureSpreadsheet() {
  ...
  const response = await googleFetch("https://sheets.googleapis.com/v4/spreadsheets", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ properties: { title: "清田さん 勤怠記録データ" }, sheets: [{ properties: { title: "勤務記録" } }] }) });
  ...
  const headers = [["記録ID", "日付", "記録状態", "勤務区分", "開始", "終了", "場所", "現場名", "住所", "緯度経度", "作業者", "作業内容", "メモ", "出張", "宿泊ホテル", "更新日時"]];
  await googleFetch(`https://sheets.googleapis.com/v4/spreadsheets/${result.spreadsheetId}/values/${encodeURIComponent("勤務記録!A1:P1")}?valueInputOption=RAW`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ values: headers }) });
...
export async function ensureDriveFolder() {
  ...
  const response = await googleFetch("https://www.googleapis.com/drive/v3/files?fields=id", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "清田さん 現場資料", mimeType: "application/vnd.google-apps.folder" }) });
