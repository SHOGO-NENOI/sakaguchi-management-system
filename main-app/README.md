...
- 音声アシスタントと、文章をまとめて解釈する入力支援
- 入力不足、時刻不整合、作業者・現場の予定重複チェック
- オフライン時の予定一時保存と、通信復帰後の自動送信
- 写真の端末内自動圧縮とGoogle Drive保存
- 毎日午前3時ごろのGoogle Drive自動バックアップと手動バックアップ
- 削除した勤務記録、現場、入力候補、道具、現場資料の復元
- 勤務記録、現場、入力候補の同時編集競合検知
- 主要な追加・更新・削除・復元・バックアップの操作履歴

自動バックアップ時刻はCloudflareのUTC cron `0 18 * * *`（日本時間の翌日午前3時）です。
Google連携が有効でない場合、Drive保存とバックアップは利用できません。
